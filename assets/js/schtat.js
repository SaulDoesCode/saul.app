(() => {
  ...
  const state = ({
    ...
    */
    const bind = (host, key, viewName, revoke) => {
      ...
      let b
      if (isFunc(key)) {
        b = viewBound ? () => key(host, view[viewName]) : () => key(host, val)
      } else {
        b = viewBound ? () => { host[key] = view[viewName] } : () => { host[key] = val }
      }
      b.revoke = () => {
        Binds.delete(host, key)
        if (revoke) revoke()
      }
      Binds.set(host, key, b, val != null)
      return b
    }
    ...
    bind.input = (input, revoke) => {
      ...
      bind.revoke = () => {
        ...
        input.removeEventListener('keydown', onchange)
        input.removeEventListener('keyup', onchange)
        input.removeEventListener('focus', onchange)
  ...
  state.collection = (states = {}) => {
    if (!isObj(states)) {
      throw new Error('state.collection needs a model object')
    }
